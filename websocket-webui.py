    ...
    try:
        async for message in websocket:
            try:
                ...
                mode = data.get('mode', '3s极速复刻')
                sft_dropdown = data.get('sft_dropdown', '')
                prompt_text = data.get('prompt_text', '')
                prompt_wav = data.get('prompt_wav', None)  # 这里可能需要额外处理音频文件
                ...
                audio_generator = generate_audio(
                    tts_text, mode, sft_dropdown, prompt_text, 
                    prompt_wav, None, instruct_text, seed, stream, speed
                )
...
async def start_websocket_server(port):
    """启动WebSocket服务器"""
    ws_port = port + 1  # WebSocket端口设为HTTP端口+1
...
def main():
    with gr.Blocks() as demo:
        gr.Markdown("### 代码库 [CosyVoice](https://github.com/FunAudioLLM/CosyVoice) \
                    ...
                    [CosyVoice-300M-Instruct](https://www.modelspace.cn/models/iic/CosyVoice-300M-Instruct) \
                    [CosyVoice-300M-SFT](https://www.modelscope.cn/models/iic/CosyVoice-300M-SFT)")
        gr.Markdown("#### 请输入需要合成的文本，选择推理模式，并按照提示步骤进行操作")
        gr.Markdown(f"**WebSocket地址**: ws://{args.host}:{args.port + 1} (用于流式语音合成)")
    ...
    # 启动Gradio和WebSocket服务器
    import threading
    
    # 在单独线程中启动WebSocket服务器
    def run_websocket_server():
        asyncio.run(start_websocket_server(args.port))
